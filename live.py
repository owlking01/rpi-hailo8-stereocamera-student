...
def main():
    ...
    while True:
        ...
        disp[disp < 0] = 0
        # --- DEBUG: disparity median in ROI --
        roi_disp = disp[ROI_Y0:ROI_Y1, ROI_X0:ROI_X1]
        valid = (roi_disp > 1.0) & np.isfinite(roi_disp)


        d_med = None
        if np.count_nonzero(valid) > 80:
            d_med = float(np.median(roi_disp[valid]))
            
        if frames % 30 == 0:
            if d_med is None or z_now is None:
                print("[LIVE] d_med=NA  Z_now=NA")
            else:
                print(f"[LIVE] d_med={d_med:.2f}px  Z_now={z_now:.2f}m")
